...
The dataset is based on Uniswap V2 event logs collected from Ethereum mainnet. The analysis covers five high-liquidity pairs:
...
For each pair, the crawler stores event data in a dedicated directory and exports separate CSV files for different event types.
...
- **Average net profit:** `$494.62`
- **Median net profit:** `$-21.64`
- **Maximum net profit:** `$34,861.72`
...
- **Mean gas ratio:** `2258.79x`
- **Median gas ratio:** `3.22x`
- **75th percentile gas ratio:** `9.59x`
...
Pair-level displacement counts are:

| Pair | Displacement Events |
| --- | ---: |
| WETH_USDC | 235 |
| WETH_USDT | 229 |
| WETH_DAI | 158 |
...
These results show that displacement behavior is concentrated in the more active markets, especially the WETH-stablecoin pairs.
...
- **Average net profit:** `$3,030.82`
- **Median net profit:** `$5.76`
- **Profitable events:** `1,943`

Pair-level arbitrage / back-running counts are:

| Pair | Arbitrage / Back-running Events |
| --- | ---: |
| WETH_USDC | 792 |
| WETH_USDT | 535 |
| WETH_DAI | 559 |
...
These results indicate that back-running is widespread across all five analyzed markets and that the profit distribution is strongly right-skewed. Most events are small, but a limited number of very profitable cases dominate the aggregate total.
...
- **Mean gas premium:** `907.12x`
- **Median gas premium:** `20.46x`

The large gap between mean and median again indicates heavy-tailed outliers.

Pair-level suppression counts are:

| Pair | Suppression Events |
| --- | ---: |
| WETH_USDC | 2 |
| WETH_USDT | 1 |
| WETH_DAI | 4 |
...
Although rare, these cases provide strong evidence of unusually aggressive priority bidding within specific blocks.
